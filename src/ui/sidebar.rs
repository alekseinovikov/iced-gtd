...
pub fn view(app: &App) -> Element<'_, Message> {
    ...
    let nav_items = [
        ...
        (View::Upcoming, "Upcoming", icons::upcoming(), Some(counts.upcoming)),
        (View::Anytime, "Anytime", icons::anytime(), Some(counts.anytime)),
    ...
    for (v, label, icon, cnt) in nav_items {
        ...
        nav_col = nav_col.push(nav_item(tokens, icon, label, cnt, active, Message::SelectView(v)));
    }
    ...
    let areas_header = container(
        row![
            ...
            container(icons::colored(icons::plus(), 12, tokens.ink_4))
                .padding(2)
        ]
        .align_y(Alignment::Center),
    )
    ...
    for area in &app.areas {
        ...
        if !collapsed {
            ...
            for p in &area.projects {
                ...
                let count_color = if active { tokens.accent_ink } else { tokens.ink_4 };
                let item = button(
                    row![
                        progress_ring::ring(progress, active, tokens),
                        text(&p.name)
                            .size(13)
                            .color(if active { tokens.accent_ink } else { tokens.ink_2 })
                            .width(Length::Fill),
    ...
    let archive_header = container(text("ARCHIVE").size(10.5).color(tokens.ink_4))
        .padding(Padding {
            top: 14.0,
            right: 12.0,
            bottom: 6.0,
    ...
    let archive_items = column![
        nav_item(tokens, icons::log(), "Logbook", None, app.view == View::Logbook && app.active_project.is_none(), Message::SelectView(View::Logbook)),
        nav_item(tokens, icons::trash(), "Trash", None, app.view == View::Trash && app.active_project.is_none(), Message::SelectView(View::Trash)),
    ]
    .spacing(2);
    ...
    // Footer
    let avatar = container(text("SK").size(12).color(tokens.on_accent))
...
fn nav_item<'a>(
    ...
    let mut row_children = vec![
        ...
        text(label).size(13.5).color(label_color).width(Length::Fill).into(),
    ];
    if let Some(c) = count {
        if c > 0 {
